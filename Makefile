...
coverage: ## coverage: Coverage.
	CONFIG_FILE=config/test.ini py.test --cov=method_specific_views --cov-report term-missing


coverage-html: ## coverage-html: Coverage with HTML report.
	CONFIG_FILE=config/test.ini py.test --cov=method_specific_views --cov-report html
...
ci: ## ci: Run CI tests.
	...
	CONFIG_FILE=config/test.ini ${PYENV_HOME}/bin/py.test --cov method_specific_views/ --cov-report term-missing --cov-report xml --cov-report html 
	${PYENV_HOME}/bin/pylint method_specific_views tests > pylint.out || exit 0
